...
import { BLOG_TABLE_ITEM_ID, blogDataSchema } from './blog-schema';
...
export const handler = async (
  ...
  const validation = blogDataSchema.safeParse(parsedBody);
  if (!validation.success) {
    return {
      statusCode: 400,
      headers,
      body: JSON.stringify({ message: 'Invalid blog data', errors: validation.error.issues }),
    };
  }
  ...
  await ddb.send(
    new PutCommand({
      TableName: tableName,
      Item: { id: BLOG_TABLE_ITEM_ID, ...validation.data },
    }),
  );
